(function () {
	...
	function loadInitialGraph() {
		const graphTypeSelection = document.getElementById("graphType");
		const graphType =
			graphTypeSelection.options[graphTypeSelection.selectedIndex].text;
	...
	function loadMetrics() {
		const processKey = document.getElementById(
			"dropdown-processList",
		).value;
		...
		if (
			...
			for (let proc of processes) {
				...
				processList.insertAdjacentHTML(
					"beforeend",
					`<vscode-option id="${processKey}" value="${processKey}">${proc.type === "local" ? proc.appName + " (pid: " + proc.pid + ")" : proc.remoteAppName + " (remote)"}</vscode-option>`,
				);
			}
		}
	}
	...
	function getMemoryData(data, memoryZones) {
		...
		dataPoint["committed"] =
			data[data.length - 2].measurements?.[0].value / 1_000_000;
		dataPoint["max"] =
			data[data.length - 1].measurements?.[0].value / 1_000_000;
		return dataPoint;
	}
	...
	function displayGraphData(graphData) {
		...
		switch (graphType) {
			case "Heap Memory":
				...
				var dataPoints = graphData.map((data) =>
					getMemoryData(data, memoryZones),
				);
	...
	function currentUsedMemory(zones, graphData) {
		return zones.map((z) => graphData[z]).reduce((v, a) => a + v, 0);
	}
	...
	function plotMemoryGraph(graphData, zones, graphType) {
		...
		chart.options.plugins.title.text = [
			graphType,
			getMetricsOverview(zones, graphData),
		];
	...
	function formula(next, prev, type) {
		if (type === "Gc Pauses") {
			return next.count < prev.count
				? (1000 * (next.total - prev.total)) / (next.count - prev.count)
				: 0;
		}
		return next.count - prev.count;
	}
	...
	function getGcMetricsOverview(unit, extraData, data) {
		...
		return (
			...
			": " +
			Math.round(
				lastData.measurements.find(
					(x) => x.statistic === extraData.prop,
				)?.value,
			) +
			unit
		);
	...
	function getGcPausesData(data) {
		const gcPause = {
			total: data.measurements.find((x) => x.statistic === "TOTAL_TIME")
				?.value,
			count: data.measurements.find((x) => x.statistic === "COUNT")
				?.value,
	...
	function plotGcGraph(graphData, type, extraData, unit, label) {
		...
		var options = {
			...
			plugins: {
				title: {
					...
					text: [
						"Gc Pauses",
						getGcMetricsOverview(unit, extraData, graphData[0]),
					],
				},
				tooltip: {
					mode: "index",
		...
		if (chartStatus === undefined) {
			...
			var gcPause = {
				total: graphData[0][0].measurements.find(
					(x) => x.statistic === "TOTAL_TIME",
				)?.value,
				count: graphData[0][0].measurements.find(
					(x) => x.statistic === "COUNT",
				)?.value,
				max: graphData[0][0].measurements.find(
					(x) => x.statistic === "MAX",
				)?.value,
			};
