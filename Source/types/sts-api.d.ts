...
export interface ExtensionAPI {
	 ...
	 */
	readonly onDidLiveProcessGcPausesMetricsUpdate: Event<
		LiveProcessPayload | string
	>;

	/**
	 * An event which fires on live process memory metrics data change. Payload is processKey.
	 */
	readonly onDidLiveProcessMemoryMetricsUpdate: Event<
		LiveProcessPayload | string
	>;
	 ...
	 */
	readonly getLiveProcessData: (
		query: SimpleQuery | BeansQuery,
	) => Promise<any>;

	/**
	 * A command to refresh live process data.
	 */
	readonly refreshLiveProcessData: (
		query: SimpleQuery | BeansQuery,
	) => Promise<any>;
	 ...
	 */
	readonly refreshLiveProcessMetricsData: (
		query: MetricsQuery,
	) => Promise<any>;
	 ...
	 */
	readonly listConnectedProcesses: () => Promise<
		(LiveProcessPayload | string)[]
	>;
}
