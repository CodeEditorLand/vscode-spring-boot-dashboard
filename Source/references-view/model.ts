...
export class BeansTreeInput implements SymbolTreeInput<BeanItem> {
	...
	constructor(
		...
		this.title =
			direction === BeansDirection.Dependencies
				? "Depending On"
				: "Injected Into";
	}
...
class BeanDataProvider implements vscode.TreeDataProvider<BeanItem> {
	...
	public async getChildren(
		element?: BeanItem | undefined,
	): Promise<BeanItem[] | undefined> {
		if (!element) {
		...
		if (this.model.direction === BeansDirection.Dependencies) {
			...
			if (element.item.dependencies?.length) {
				for (const dep of element.item.dependencies) {
					...
					if (details?.length) {
						children.push({
							...details[0],
							processKey: element.item.processKey,
						});
					} else {
						children.push({
							id: dep,
			...
			// Injected Into
			const beans: Bean[] = await getBeansDependingOn(
				element.item.processKey,
				element.item.id,
			);
			if (beans?.length) {
