	...
	if (cp.entries) {
		...
		for (let i = 0; i < entries.length; i++) {
			...
			if (
				...
				filename.startsWith("spring-boot")
			) {
				return true;
			}
		}
...
export class LocalAppManager {
	...
	constructor() {
		...
		this._startAppListSynchronisation().catch((error) => {
			console.error(error);
		});
	}
	...
	public getAppList(): BootApp[] {
		return Array.from(this._boot_projects.values()).sort((a, b) =>
	...
	public getAppByMainClass(mainClass: string): BootApp | undefined {
		return this.getAppList().find((app) =>
			app.mainClasses?.find(
				(mcd: MainClassData) => mcd.mainClass === mainClass,
			),
		);
	}
			...
			while (available_tries > 0) {
				...
				try {
					...
					if (available_tries > 0) {
						...
						throw new Error(
							`Failed to register classpath listener after ${MAX_RETRIES} retries.`,
						);
					}
				}
			}
