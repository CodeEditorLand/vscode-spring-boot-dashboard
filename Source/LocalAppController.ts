...
export class LocalAppController {
			...
			if (appsToRun !== undefined) {
				...
				await Promise.all(
					appList
						...
						.map((app) => this.runBootApp(app, debug)),
				);
			}
		}
	}

	public async runBootApp(
		app: BootApp,
		debug?: boolean,
		profile?: string,
	): Promise<void> {
		const mainClasData = await vscode.window.withProgress(
			{
				...
				title: `Resolving main classes for ${app.name}...`,
			},
			async () => {
				...
				if (
					mainClassList &&
					mainClassList instanceof Array &&
					mainClassList.length > 0
				) {
					return mainClassList.length === 1
						? mainClassList[0]
						: await vscode.window.showQuickPick(
								mainClassList.map((x) =>
									Object.assign({ label: x.mainClass }, x),
								),
								{
	...
	public onDidStartBootApp(session: vscode.DebugSession): void {
		// exact match
		let app: BootApp | undefined = this.manager
			.getAppList()
			.find((elem: BootApp) => elem.activeSessionName === session.name);
		...
		if (app === undefined) {
			app = this.manager
				...
				.find(
					(elem: BootApp) =>
						elem.name === session.configuration.projectName,
				);
		}
	...
	public async stopBootApps() {
		...
		if (appList.length === 1 && appList[0].state !== AppState.INACTIVE) {
			...
			const appsToStop = await vscode.window.showQuickPick(
				appList
					.filter((app) => app.state !== AppState.INACTIVE)
					.map((app) => ({
						label: app.name,
						path: app.path,
					})) /** items */,
	...
	private async getOpenUrlFromJMX(app: BootApp) {
		...
		READ_JMX_EXTENSION_RESPONSE: {
			if (stdout !== null) {
				...
				if (
					jmxExtensionResponse["local.server.port"] !== null &&
					typeof jmxExtensionResponse["local.server.port"] ===
						"number"
				) {
				...
				if (
					jmxExtensionResponse["server.servlet.context-path"] !== null
				) {
					contextPath =
						jmxExtensionResponse["server.servlet.context-path"];
				}

				if (
					jmxExtensionResponse["status"] !== null &&
					jmxExtensionResponse["status"] === "failure"
				) {
	...
	public async openBootApp(app: BootApp): Promise<void> {
		...
		if (openUrl !== undefined) {
			const openWithExternalBrowser: boolean =
				vscode.workspace
	...
	private _getLaunchConfig(mainClasData: MainClassData) {
		...
		return rawConfigs.find(
			(conf) =>
				conf.type === "java" &&
				conf.request === "launch" &&
				conf.mainClass === mainClasData.mainClass &&
				conf.projectName === mainClasData.projectName,
		);
	}
	...
	private async _createNewLaunchConfig(
		mainClasData: MainClassData,
	): Promise<vscode.DebugConfiguration> {
		const newConfig = {
			type: "java",
			name: this._constructLaunchConfigName(
				mainClasData.mainClass,
				mainClasData.projectName,
			),
			request: "launch",
			cwd: "${workspaceFolder}",
...
function isRunInTerminal(session: vscode.DebugSession) {
	return (
		...
		session.configuration.console !== "internalConsole"
	);
}

function isActuatorOnClasspath(
...
async function resolveDebugConfigurationWithSubstitutedVariables(
	debugConfiguration: vscode.DebugConfiguration,
): Promise<vscode.DebugConfiguration> {
	...
	if (
		debugConfiguration.vmArgs.indexOf("-Dcom.sun.management.jmxremote") < 0
	) {
