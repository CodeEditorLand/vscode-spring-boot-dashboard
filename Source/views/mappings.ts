...
export class Endpoint {
	...
	constructor(processKey: string, rawDataMap?: any) {
	...
	get pattern(): string | undefined {
		return this.rawDataMap?.details?.map?.requestMappingConditions?.map
			?.patterns?.myArrayList?.[0];
	}

	get method(): string | undefined {
		return this.rawDataMap?.details?.map?.requestMappingConditions?.map
			?.methods?.myArrayList?.[0];
	}
...
export class MappingsDataProvider implements vscode.TreeDataProvider<TreeData> {
	...
	private onDidRefreshMappings: vscode.EventEmitter<TreeData | undefined> =
		new vscode.EventEmitter<TreeData | undefined>();

	constructor() {
		vscode.commands.executeCommand(
			"setContext",
			"spring.mappings:showMode",
			"defined",
		);
	}
	...
	async getChildren(element?: TreeData): Promise<TreeData[] | undefined> {
		...
		if (!element) {
			...
			vscode.commands.executeCommand(
				"setContext",
				"spring.mappings:hasLiveProcess",
			...
			ret.sort((a, b) =>
				(
					(a as LiveProcess).appName ?? (a as BootApp).name
				).localeCompare(
			...
			if (correspondingApp) {
				...
				if (staticMappings?.length) {
					liveMappings?.forEach(
						(lm) =>
							(lm.corresponding = staticMappings?.find(
								(sm) => sm.label === lm.label,
							)),
					);
				}
	...
	public getParent(element: TreeData): vscode.ProviderResult<TreeData> {
		if (element instanceof LiveProcess) {
			...
			return Array.from(this.staticData.keys()).find((k) =>
				this.staticData.get(k)?.includes(element),
			);
		}
	...
	public refreshLive(
		liveProcess: sts.LiveProcessPayload,
		mappingsRaw: any[] | undefined,
	) {
	...
	public updateStaticData(app: BootApp, mappingsRaw: SymbolInformation[]) {
		const mappings = mappingsRaw
			.map((raw) => new StaticEndpoint(raw))
			.sort((a, b) => a.label.localeCompare(b.label));
...
export async function openEndpointHandler(endpoint: Endpoint) {
	...
	let url: string | undefined = constructOpenUrl(
		contextPath,
		port,
		endpoint.pattern,
		hostname,
