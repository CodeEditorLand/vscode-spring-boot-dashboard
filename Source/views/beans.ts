...
export class BeansDataProvider implements vscode.TreeDataProvider<TreeData> {
	...
	private onDidRefreshBeans: vscode.EventEmitter<TreeData | undefined> =
		new vscode.EventEmitter<TreeData | undefined>();

	constructor() {
		vscode.commands.executeCommand(
			"setContext",
			"spring.beans:showMode",
			"defined",
		);
	}
	...
	public updateStaticData(app: BootApp, beansRaw: SymbolInformation[]) {
		const mappings = beansRaw
			.map((raw) => new StaticBean(raw))
			.sort((a, b) => a.id.localeCompare(b.id));
	...
	if (bean.type) {
		...
		if (uriString) {
			await vscode.commands.executeCommand(
				...
				{ preserveFocus: true, selection: range },
			);
		}
	}
}
